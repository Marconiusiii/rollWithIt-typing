...
Never gonna tell a lie and hurt you`;
        ...
        const punctuationMap = {
            "'": "apostrophe", ",": "comma", ".": "period", "-": "hyphen",
            "!": "exclamation mark", "?": "question mark", " ": "space"
        };
        ...
        async function generateCustomLyrics() {
            ...
            try {
                const result = await callGemini(topic, "Generate 6-8 short lines of lyrics for a typing game. No labels.");
                if (result.trim()) {
        ...
        function speak(text) {
            return new Promise((resolve) => {
                ...
                // Voice selection optimization for iOS
                const voices = window.speechSynthesis.getVoices();
                const enVoice = voices.find(v => v.lang.includes('en') && v.name.includes('Samantha')) || voices[0];
        ...
        function playBeep() {
            ...
            osc.frequency.value = 220; gain.gain.value = 0.05;
        ...
        function render() {
            ...
            for (let i = 0; i < line.length; i++) {
                ...
                if (i < currentCharIndex) span.className = 'correct';
                else if (i === currentCharIndex) span.className = 'current';
                container.appendChild(span);
            }
            if (currentCharIndex === line.length) {
                container.appendChild(Object.assign(document.createElement('span'), {className: 'cursor'}));
            }
        }

        async function promptChar() {
            const char = lines[currentLineIndex][currentCharIndex];
            const p = punctuationMap[char] || (char === char.toUpperCase() && char !== char.toLowerCase() ? `Capital ${char}` : char);
            document.getElementById('char-indicator').textContent = `Type: ${p}`;
            // Local speak is immediate
            speak(p);
        }
        ...
        async function processKey(key) {
            ...
            if (gameState === 'PLAYING') {
                if (key === '\\') { finishGame(); return; }
                
                // Note: We don't block on isSpeaking here anymore for a snappier feel, 
                // but we clear previous speech in the speak() function.
        ...
        async function finishGame() {
            ...
            const acc = Math.round(((totalKeystrokes - errors) / Math.max(1, totalKeystrokes)) * 100) || 0;
            ...
            const review = await callGemini(`Speed: ${wpm}, Acc: ${acc}%. Give 2 sentences of encouragement.`, "Friendly typing coach.");
            document.getElementById('ai-feedback-text').textContent = review;
            speak(review);
        }

        document.getElementById('generate-btn').addEventListener('click', generateCustomLyrics);
        
        // Populate voices list for iOS on load
